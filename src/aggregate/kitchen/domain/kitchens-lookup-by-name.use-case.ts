...
export class KitchensLookupByNameUseCase {
  constructor(
    private readonly kitchenLookupRepository: KitchenLookupRepository,
  ) {}

  async execute(kitchenName: string): Promise<Result<Kitchen[] | ApiError>> {
    try {
      kitchenName = kitchenName.trim();

      const kitchen = await this.kitchenLookupRepository.getKitchensByName(kitchenName);
